...
To write a `GatewayFilter`, you must implement `TimeGatewayFilterFactory`.
...
Custom filters class names should end in `TimeGatewayFilterFactory`.
...
`TimeGatewayFilterFactory` suffix, such as `class AnotherThing`. This filter could be
